...
int main (int argc, char*argv[]) {
	...
	strcpy(buf, prog);									//kopiuje z bufora do zmiennej
    wysyla(dg,buf,100);									//wysy³¹ deskryptor i bufor
    fflush(stdout);
    odbiera(dg,buf,100);								//odbiera od serwera deskryptor i bufor
    puts(buf);											//wypisuje bufor
    close (dg);
    return 0;
}
